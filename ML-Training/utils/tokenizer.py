...
class SentencePieceTokenizer:
    ...
    def __init__(self, model_file: str, max_length: int = 128):
        ...
        self.cls_token_id = self.sp_model.PieceToId("[CLS]")
        self.sep_token_id = self.sp_model.PieceToId("[SEP]")
        self.mask_token_id = self.sp_model.PieceToId("[MASK]")

    def __call__(self, text, max_length=None, padding="max_length", truncation=True, return_tensors="pt", **kwargs):
        """Tokenize text"""
        ...
        for t in text:
            # Tokenize
            token_ids = self.sp_model.EncodeAsIds(t)

            # Truncate
            if truncation and len(token_ids) > max_length - 2:  # -2 for CLS and SEP
    ...
    def batch_decode(self, token_ids_list, skip_special_tokens=True):
        ...
        return [self.sp_model.DecodeIds(ids) for ids in token_ids_list]
...
def create_sentencepiece_tokenizer(
    ...
    **kwargs,
    ...
    if model_path.exists():
        ...
        tokenizer = SentencePieceTokenizer(str(model_path), max_length=128)
        return tokenizer

    # Train new SentencePiece model
    if train_data is None or len(train_data) == 0:
        raise ValueError(
            "❌ No training data provided for SentencePiece tokenizer!\n"
            "   Please provide training texts via train_data parameter.\n"
            "   Or use a pretrained tokenizer: solar, exaone, kobert"
        )
